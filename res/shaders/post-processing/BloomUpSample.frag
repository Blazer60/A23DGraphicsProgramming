...
uniform vec2 u_mip_size;  // todo: Change this to texture size.
...
void main()
{
    ...
    o_output = max(up_filter_9_tap(u_up_sample_texture, v_uvs, texel_size), vec3(0.f))
             + textureLod(u_down_sample_texture, v_uvs, u_down_mip_level).rgb;
}
