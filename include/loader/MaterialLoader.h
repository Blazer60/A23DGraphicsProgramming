...
namespace load
{
    typedef std::function<void(std::string&, MtlMaterial &)> generateMaterialSignature;
    ...
    Materials<TMaterial> material(std::string_view path)
    {
        ...
        parseMaterial(path, [&](const std::string &materialName, const MtlMaterial &mtlMaterial) {
